...
$edit="SELECT*FROM tb_surat_keterangan where id_surat='$id_surat'";
...
<div class="w-50 mx-auto border p-3 mt-5">
	<h4 class="mb"><i class="fa fa-angle-right"></i> Edit Data Mahasiswa</h4>
		<form action="edit.php" method="POST" enctype="multipart/form-data">
			...
			<input type="text" name="id_surat" class="form-control" value="<?php echo $data['id_surat'];?>" readonly>

			<label>Id Verifikasi</label>
			<select name="id_verifikasi" id="id_verifikasi" class="form-control" value="<?php echo $data['id_verifikasi'];?>" required="required">
                        <option value="">--Id Verifikasi--</option>
                        <?php 
                        $sql_nama = mysqli_query($conn, "SELECT * FROM tb_verifikasi") or die (mysqli_error($conn));
                        while ($data_nama = mysqli_fetch_array($sql_nama)) {
                        echo '<option value ="'.$data_nama['id_verifikasi'].'">'.$data_nama['id_verifikasi'].'</option>';
                        }
                        ?>
                      </select>
			<label>NIP</label>
			<select name="nip" id="nip" class="form-control" value="<?php echo $data['nip'];?>" required="required">
                        <option value="">--Nama Pegawai--</option>
                        <?php 
                        $sql_nama = mysqli_query($conn, "SELECT * FROM tb_pegawai") or die (mysqli_error($conn));
                        while ($data_nama = mysqli_fetch_array($sql_nama)) {
