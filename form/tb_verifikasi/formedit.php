...
$edit="SELECT*FROM tb_verifikasi where id_verifikasi='$id_verifikasi'";
...
<div class="w-50 mx-auto border p-3 mt-5">
	<h4 class="mb"><i class="fa fa-angle-right"></i> Edit Data Mahasiswa</h4>
		<form action="edit.php" method="POST" enctype="multipart/form-data">
			...
			<input type="text" name="id_verifikasi" class="form-control" value="<?php echo $data['id_verifikasi'];?>" readonly>

			<label>Id Pengajuan</label>
      		<select name="id_pengajuan" id="id_pengajuan" class="form-control" required="required">
                        <option value="">--Id Pengajuan--</option>
                        <?php 
                        $sql_nama = mysqli_query($conn, "SELECT * FROM tb_pengajuan") or die (mysqli_error($conn));
                        while ($data_nama = mysqli_fetch_array($sql_nama)) {
                        echo '<option value ="'.$data_nama['id_pengajuan'].'">'.$data_nama['id_pengajuan'].'</option>';
                        }
                        ?>
                      </select>
     		...
     		<input type="date" name="tgl_keluar"  class="form-control" value="<?php echo $data['tgl_keluar'];?>" required>
